...
public class ElasticSearchExtension extends LifecycleAdapter {
    ...
    public ElasticSearchExtension(GraphDatabaseService gds, String hostName, String userName,String pwd,String indexSpec, Boolean discovery, Boolean includeIDField, Boolean includeLabelsField) {
        Map iSpec;
        try {
            iSpec = ElasticSearchIndexSpecParser.parseIndexSpec(indexSpec);
            if (iSpec.size() == 0) {
...
//        client = getJestClient(hostName, discovery);
        client = getJestClient(hostName, userName,pwd,discovery);
    ...
    private JestClient getJestClient(final String hostName,final String userName,final String pwd, final Boolean discovery) throws Throwable {
        JestClientFactory factory = new JestClientFactory();
        factory.setHttpClientConfig(JestDefaultHttpConfigFactory.getConfigFor(hostName,userName,pwd, discovery));
        return factory.getObject();
    }
